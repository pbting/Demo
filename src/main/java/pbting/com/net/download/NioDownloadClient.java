...
public class NioDownloadClient {
	...
	static CharsetDecoder decoder = Charset.forName("utf-8").newDecoder();//coder();
	...
	static class Download implements Runnable {
		...
		public void run() {
			try {
				...
				int total = 0;
				BufferedWriter writer = new BufferedWriter(new FileWriter("D:/demo_1.txt"));
				FOR: for (;;) {
					selector.select();
					...
					while (iter.hasNext()) {
						...
						if (key.isConnectable()) {
							...
							if (count > 0) {
								total += count;
								buffer.flip();
								String message = NioDeEnCoder.getVarStrFromBuf(buffer, 0, buffer.limit());
